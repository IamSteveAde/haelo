  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ width: 36, height: 36, borderRadius: 9, background: hov ? INK : INK_06, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .22s' }}>
          <s.icon size={16} color={hov ? '#fff' : INK_40} />
        </div>
        {s.up && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 10, fontWeight: 700, color: GREEN, background: GREEN_BG, padding: '3px 7px', borderRadius: 20 }}>
            <TrendingUp size={10} /> Up
          </div>
        )}
      </div>
      <p style={{ fontSize: 28, fontWeight: 800, color: INK, letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 4 }}>{s.value}</p>
      <p style={{ fontSize: 11, fontWeight: 600, color: INK_60, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.05em' }}>{s.label}</p>
...
function ActivityRow({ item }: { item: typeof recentActivity[0] }) {
  ...
  return (
    <div
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', borderRadius: 12, background: hov ? CREAM : 'transparent', transition: 'all .18s cubic-bezier(.4,0,.2,1)', cursor: 'default' }}
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: 12, color: INK_60, lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{item.summary}</p>
...
export default function OverviewPage() {
  ...
  const [statusHov, setStatusHov]     = useState(false)
  ...
  return (
    <>
      ...
      <main style={{ flex: 1, padding: '40px 40px 60px', overflowY: 'auto', maxWidth: 1200, margin: '0 auto', width: '100%' }}>
        ...
        {/* ── HEADER ROW: greeting left, alert right ── */}
        <div className="fade-up" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, marginBottom: 32 }}>
          ...
          {/* Alert — top right, same row as greeting */}
          <AlertBadge />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 12, alignItems: 'start' }}>
          ...
          {/* LEFT: Activity feed */}
          <Card hov={activityHov} onEnter={() => setActivityHov(true)} onLeave={() => setActivityHov(false)} style={{ height: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div
              ...
              onMouseEnter={() => setStatusHov(true)}
              onMouseLeave={() => setStatusHov(false)}
              style={{
                background: NAVY, borderRadius: 16, padding: '22px 24px',
                transition: 'all .22s cubic-bezier(.4,0,.2,1)',
                boxShadow: statusHov ? '0 10px 36px rgba(10,22,40,0.28)' : '0 2px 8px rgba(10,22,40,0.12)',
                transform: statusHov ? 'translateY(-1px)' : 'none',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
                <div style={{ width: 7, height: 7, background: '#4ABA7A', borderRadius: '50%', animation: 'breathe 2.2s ease-in-out infinite' }} />
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#4ABA7A' }}>Active</span>
              </div>
              ...
              <a
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: 'rgba(74,186,122,0.15)', color: '#4ABA7A', fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 12, fontWeight: 700, padding: '10px', borderRadius: 10, border: '1px solid rgba(74,186,122,0.2)', textDecoration: 'none', transition: 'all .18s', letterSpacing: '.01em' }}
              >
