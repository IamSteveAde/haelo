  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? INK_20 : hov ? '#1a3a12' : INK,
        color: disabled ? INK_40 : '#fff',
        fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 13, fontWeight: 600,
        padding: '12px 22px', borderRadius: 10, border: 'none',
        cursor: disabled ? 'not-allowed' : 'pointer',
        transform: hov && !disabled ? 'translateY(-1px)' : 'none',
        boxShadow: hov && !disabled ? '0 6px 20px rgba(17,39,11,0.22)' : 'none',
...
function BtnGreen({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  ...
  return (
    <button
      ...
      style={{
        display: 'inline-flex', alignItems: 'center', gap: 8,
        background: hov ? '#236040' : GREEN, color: '#fff',
        fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 13, fontWeight: 600,
        padding: '12px 22px', borderRadius: 10, border: 'none', cursor: 'pointer',
        transform: hov ? 'translateY(-1px)' : 'none',
        boxShadow: hov ? '0 6px 18px rgba(46,125,82,0.3)' : 'none',
        transition: 'all .2s cubic-bezier(.4,0,.2,1)',
      }}
    >
      {children}
...
function BtnGhost({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 12, fontWeight: 600, color: hov ? INK : INK_40, padding: 0, transition: 'color .15s' }}
    >
      {children}
    </button>
  )
    ...
    <div style={{ position: 'relative' }}>
      <select
        ...
        style={{
          ...
          borderRadius: 10, padding: '11px 38px 11px 14px', fontSize: 13,
          fontWeight: 600, color: INK, outline: 'none', cursor: 'pointer',
          boxShadow: foc ? `0 0 0 3px ${INK_06}` : 'none', transition: 'all .18s',
        }}
      >
        {children}
      </select>
      <svg
        style={{ position: 'absolute', right: 13, top: '50%', transform: `translateY(-50%) rotate(${foc ? '-180' : '0'}deg)`, pointerEvents: 'none', transition: 'transform .2s' }}
...
function Eyebrow({ n }: { n: number }) {
  return <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: GOLD, marginBottom: 8 }}>Step {n} of 5</p>
}

function StepTitle({ children }: { children: React.ReactNode }) {
  return <h2 style={{ fontSize: 26, fontWeight: 700, color: INK, lineHeight: 1.15, letterSpacing: '-.025em', marginBottom: 6 }}>{children}</h2>
}

function StepSub({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 13, color: INK_60, lineHeight: 1.6, marginBottom: 28 }}>{children}</p>
}
...
const STEP_LABELS = ['Email', 'WhatsApp', 'Business Bible', 'Tone', 'Timer']
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                background: done || active ? INK : CREAM,
                border: `1.5px solid ${done || active ? INK : INK_20}`,
                color: done || active ? '#fff' : INK_40,
                boxShadow: active ? `0 0 0 4px ${INK_06}` : 'none',
                transition: 'all .35s cubic-bezier(.4,0,.2,1)',
              }}>
                {done ? '✓' : i + 1}
              </div>
              <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', whiteSpace: 'nowrap', color: active ? INK : done ? INK_60 : INK_20, transition: 'color .3s' }}>
                {label}
              </span>
            </div>
            {i < STEP_LABELS.length - 1 && (
              <div style={{ height: 1, width: 68, flexShrink: 0, background: done ? INK : INK_10, marginTop: 15, marginLeft: 4, marginRight: 4, transition: 'background .5s cubic-bezier(.4,0,.2,1)' }} />
            )}
          </div>
        )
      })}
...
function Sidebar({ step }: { step: number }) {
  ...
  return (
    <div style={{ background: NAVY, borderRadius: 16, padding: '28px 24px', position: 'sticky', top: 80 }}>
      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: GOLD_LIGHT, marginBottom: 6 }}>Step {step + 1} of 5</p>
...
// ── STEP 0: EMAIL ─────────────────────────────────────────────────────────────
// Each provider is its own component so useState is called at the top level
  ...
  return (
    <button
      ...
      style={{
        ...
        background: isMe || hov ? WHITE : CREAM,
        border: `1.5px solid ${isMe ? INK : hov ? INK_20 : INK_10}`,
        borderRadius: 12, cursor: connecting ? 'default' : 'pointer', width: '100%',
        fontFamily: "'Plus Jakarta Sans', sans-serif", marginBottom: 8,
        transform: hov && !connecting ? 'translateY(-1px)' : 'none',
        boxShadow: hov && !connecting ? `0 2px 12px ${INK_06}` : 'none',
        transition: 'all .2s cubic-bezier(.4,0,.2,1)',
      }}
    >
      <div style={{ width: 38, height: 38, borderRadius: 9, background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
        {letter}
      </div>
      ...
      {isMe
        ? <Spinner />
        : (
          <svg style={{ marginLeft: 'auto', color: hov ? INK : INK_20, transition: 'all .2s', transform: hov ? 'translateX(3px)' : 'none' }} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round">
...
function StepEmail({ onNext }: { onNext: () => void }) {
  ...
  if (connected) {
    ...
    return <SuccessState emoji="✅" eyebrow="Connected" title={`${name} is connected.`} sub="Haelo is now watching your inbox in real time." cta="Continue to WhatsApp" onCta={onNext} green />
  }
  ...
  return (
    <div>
      ...
      {providers.map(p => (
        <ProviderBtn key={p.id} {...p} connecting={connecting} onConnect={connect} />
      ))}
  ...
  if (sent) {
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {code.map((digit, i) => (
            <input
              ...
              style={{ flex: 1, textAlign: 'center', fontSize: 22, fontWeight: 700, color: INK, background: digit ? WHITE : CREAM, border: `1.5px solid ${digit ? INK_40 : INK_10}`, borderRadius: 10, padding: '12px 0', outline: 'none', fontFamily: "'Plus Jakarta Sans', sans-serif", transition: 'all .18s' }}
            />
          ))}
        </div>
        {verifying && (
        ...
        <BtnGhost onClick={() => { setSent(false); setCode(['', '', '', '', '', '']) }}>Resend code</BtnGhost>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: INK_40, marginBottom: 5 }}>WhatsApp number</label>
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ background: CREAM, border: `1.5px solid ${INK_10}`, borderRadius: 10, padding: '11px 14px', fontSize: 13, fontWeight: 700, color: INK, whiteSpace: 'nowrap', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
            🇳🇬 +234
          </div>
          <input
            type="tel" placeholder="801 234 5678" value={phone}
            onChange={e => setPhone(e.target.value.replace(/\D/g, ''))}
            onFocus={() => setPhoneFoc(true)}
            onBlur={() => setPhoneFoc(false)}
            style={{ flex: 1, fontFamily: "'Plus Jakarta Sans', sans-serif", background: phoneFoc ? WHITE : CREAM, border: `1.5px solid ${phoneFoc ? INK : INK_10}`, borderRadius: 10, padding: '11px 14px', fontSize: 13, color: INK, outline: 'none', boxShadow: phoneFoc ? `0 0 0 3px ${INK_06}` : 'none', transition: 'all .18s' }}
          />
        </div>
      </div>

      <div style={{ background: NAVY, borderRadius: 14, padding: 18, marginBottom: 20 }}>
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.09em', textTransform: 'uppercase', color: GOLD_LIGHT, marginBottom: 12 }}>What you'll receive</p>
        <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: 14 }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: '#4ABA7A', marginBottom: 8 }}>New Internal Email</p>
...
const BIBLE_DOCS = [
  ...
  },
  {
    ...
    hint: 'Every person Haelo will recognise. Each row must include first name, last name, role, email, and a one-sentence description of their responsibilities.',
  ...
  {
    id: 'org', icon: '🏗️', name: 'Org Chart / Structure', required: true, isStaff: false,
    hint: 'Your hierarchy and reporting lines. Haelo uses this to understand seniority, route responses correctly, and handle escalations.',
  ...
  },
  {
    ...
    hint: 'Approval thresholds, leave policies, procurement rules, escalation paths. The more detail, the more accurate every Haelo response.',
  ...
  {
    id: 'comms', icon: '💬', name: 'Communication Style', required: false, isStaff: false,
    hint: 'How you communicate with your team. Formal or direct? Long or brief? Phrases you use or avoid? Haelo mirrors this in every draft.',
    accept: '.pdf,.docx,.doc,.txt',
...
// Each file row is its own component — keeps hover state at top level
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: CREAM, border: `1.5px solid ${file.processing ? INK_20 : INK_10}`, borderRadius: 10 }}>
      ...
      {!file.processing && (
        <button
          ...
          onMouseEnter={() => setHov(true)}
          onMouseLeave={() => setHov(false)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: hov ? '#C0392B' : INK_20, fontSize: 16, padding: '2px 5px', borderRadius: 5, lineHeight: 1, transition: 'all .15s', flexShrink: 0 }}
        >×</button>
      )}
    </div>
  )
...
// Each bible tab is its own component
  ...
  return (
    <div>
      ...
      {doc.isStaff && (
        <div style={{ background: NAVY, borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: GOLD_LIGHT }}>📥 Staff Directory Template</span>
          ...
          <div style={{ border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, overflow: 'hidden', marginBottom: 10 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', background: 'rgba(255,255,255,0.1)' }}>
              {['First Name', 'Last Name', 'Role', 'Email', 'Description'].map(col => (
                <div key={col} style={{ padding: '7px 10px', fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.6)', borderRight: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              {['Tosin', 'Adeyemi', 'Ops Manager', 'tosin@co.com', 'Manages daily operations…'].map((cell, i) => (
                <div key={i} style={{ padding: '7px 10px', fontSize: 11, color: 'rgba(255,255,255,0.38)', fontStyle: 'italic', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
                  {cell}
                </div>
              ))}
            </div>
      ...
      {!doc.isStaff && doc.templateCols && (
        <div style={{ background: NAVY, borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: GOLD_LIGHT }}>📥 {doc.name} Template</span>
            <button
              onClick={() => downloadCSV(doc.templateFile!, doc.templateCols!, doc.templateRows!)}
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '11px 14px', marginBottom: 10 }}>
            <div style={{ display: 'grid', gridTemplateColumns: doc.templateCols.map(() => '1fr').join(' '), gap: 12, paddingBottom: 6, marginBottom: 6, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              {doc.templateCols.map(c => (
                <span key={c} style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>{c}</span>
              ))}
            </div>
      ...
      <div
        ...
        style={{ border: `2px dashed ${dragOver ? INK : INK_20}`, borderRadius: 12, padding: '30px 20px', textAlign: 'center', cursor: 'pointer', background: dragOver ? INK_03 : 'transparent', transition: 'all .2s', marginBottom: 12 }}
      >
        <input ref={fileRef} type="file" accept={doc.accept} multiple style={{ display: 'none' }} onChange={e => { if (e.target.files) handleFiles(Array.from(e.target.files)) }} />
...
// Each tone card is its own component — no hooks in map
function ToneCard({ tone, active, onClick }: { tone: typeof TONES[0]; active: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      onMouseLeave={() => setHov(false)}
      style={{
        display: 'flex', flexDirection: 'column', gap: 3, padding: '13px 15px',
        background: active || hov ? WHITE : CREAM,
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 18 }}>
        {TONES.map(t => (
          <ToneCard key={t.id} tone={t} active={selected === t.id} onClick={() => setSelected(t.id)} />
        ))}
      </div>

      {selected === 'custom' && (
        <div style={{ marginBottom: 18 }}>
          <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: INK_40, marginBottom: 5 }}>Describe your tone</label>
          <textarea
            value={customText}
            onChange={e => setCustomText(e.target.value)}
            onFocus={() => setTextFoc(true)}
            onBlur={() => setTextFoc(false)}
            placeholder='e.g. Warm but brief. Always acknowledge before directing. Never start a reply with "I". Avoid corporate language and filler words.'
            rows={3}
...
// Each timer option is its own component — no hooks in map
function TimerOpt({ opt, active, onClick }: { opt: typeof TIMER_OPTS[0]; active: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      onMouseLeave={() => setHov(false)}
      style={{
        display: 'flex', alignItems: 'flex-start', gap: 12, padding: '15px 16px',
        background: active || hov ? WHITE : CREAM,
        border: `1.5px solid ${active ? INK : hov ? INK_20 : INK_10}`,
    ...
    >
      <div style={{ width: 34, height: 34, borderRadius: 9, background: opt.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0, marginTop: 1 }}>
        {opt.icon}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 2 }}>
          ...
          {opt.tag && (
            <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 8, background: GOLD_BG, color: GOLD, border: `1px solid ${GOLD_BORDER}` }}>
              {opt.tag}
            </span>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 18 }}>
        {TIMER_OPTS.map(o => (
          <TimerOpt key={o.id} opt={o} active={mode === o.id} onClick={() => setMode(o.id)} />
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: mode !== 'auto-send' ? '1fr 1fr' : '1fr', gap: 12, marginBottom: 22 }}>
        <div>
          <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: INK_40, marginBottom: 5 }}>Auto-send after</label>
        ...
        {mode !== 'auto-send' && (
          <div>
            <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: INK_40, marginBottom: 5 }}>Reminder every</label>
...
function StepReady() {
  ...
  const checks = ['Email connected', 'WhatsApp verified', 'Business Bible uploaded', 'Response tone set', 'Timer configured', 'Monitoring active']
  ...
  return (
    <div style={{ textAlign: 'center', padding: '28px 0' }}>
      ...
      <h2 style={{ fontSize: 32, fontWeight: 800, color: INK, lineHeight: 1.1, letterSpacing: '-.03em', marginBottom: 10 }}>
...
export default function OnboardingPage() {
  ...
  return (
    <>
      ...
      <header style={{ background: NAVY, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'sticky', top: 0, zIndex: 200 }}>
        <HaeloLogo />
      ...
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '44px 24px 80px' }}>
        ...
        {step < 5 ? (
          <div key={step} className="fade-up" style={{ display: 'grid', gridTemplateColumns: '248px 1fr', gap: 20, alignItems: 'start' }}>
            <Sidebar step={step} />
            <div style={{ background: WHITE, borderRadius: 16, border: `1px solid ${INK_10}`, padding: 36, boxShadow: '0 1px 24px rgba(17,39,11,0.06)' }}>
              {step === 0 && <StepEmail onNext={next} />}
              {step === 1 && <StepWhatsApp onNext={next} onBack={back} />}
              {step === 2 && <StepBible onNext={next} onBack={back} />}
              {step === 3 && <StepTone onNext={next} onBack={back} />}
              {step === 4 && <StepTimer onNext={next} onBack={back} />}
            </div>
          </div>
        ) : (
          <div key={5} className="fade-up" style={{ background: WHITE, borderRadius: 16, border: `1px solid ${INK_10}`, padding: 48, boxShadow: '0 1px 24px rgba(17,39,11,0.06)', maxWidth: 600, margin: '0 auto' }}>
            <StepReady />
          </div>
        )}
      </div>
